...
local COLORS = {
  ...
  wall = { 0.86, 0.86, 0.79, 1 },
  side = { 0.72, 0.73, 0.67, 1 },
  roof = { 0.92, 0.92, 0.88, 1 },
  roofFar = { 0.78, 0.79, 0.75, 1 },
  fascia = { 0.48, 0.49, 0.45, 1 },
  door = { 0.92, 0.92, 0.87, 1 },
}
...
local function rgba(c, mul, alpha)
  mul = mul or 1
  love.graphics.setColor(c[1] * mul, c[2] * mul, c[3] * mul, alpha or c[4] or 1)
end
...
function BuildingRenderer:update(dt, level)
...
function BuildingRenderer:prepareScene(map)
  ...
  self.lastSceneBuilds = self.SceneBuilder.buildCount
  return prepared
end
...
function BuildingRenderer:drawGround(proj, prepared)
  ...
  for i = 1, #prepared.ground do
    ...
    }, c.texture and { 1, 1, 1, 1 } or COLORS.ground)
    self.lastGroundCells = self.lastGroundCells + 1
  end
end

function BuildingRenderer:drawBuildingShadow(proj, p)
...
function BuildingRenderer:drawBuilding(proj, preparedBuilding)
  local p = preparedBuilding.semantic
  local f, a = p.footprint, p.architecture
...
function BuildingRenderer:collectActors(state, proj)
  local out, seen = {}, {}
  local entities = state and state.entities or {}
  local i
  for i = 1, #entities do
    local row = actorPose(entities[i])
    if row then
      seen[row.actor] = true
...
function BuildingRenderer:drawActor(proj, row)
  ...
  local shx, shy = proj:worldPixel(wx, wy, 0)
  local s = proj.spriteScale
  love.graphics.setColor(0, 0, 0, 0.16)
  love.graphics.ellipse("fill", shx, shy + 1, 5.2 * s, 1.9 * s)
  love.graphics.setColor(1, 1, 1, 1)
