...
local function scaledProjection(proj, factor)
  if not proj or math.abs((factor or 1) - 1) < 0.0001 then return proj end
  local proxy = {
    level = proj.level, tileW = proj.tileW, tileH = proj.tileH,
...
local function ensureRockMesh(renderer)
  if renderer.naturalShapeRockMesh then return renderer.naturalShapeRockMesh end
  ...
  renderer.naturalShapeRockMesh = mesh
  return mesh
end
...
function NaturalShapePolish.apply(renderer)
  if not renderer or renderer.__naturalShapePolishApplied then return renderer end
  renderer.__naturalShapePolishApplied = true
  ...
  renderer.invalidate = function(self)
    if self.naturalShapeRockMesh and self.naturalShapeRockMesh.release then
      pcall(self.naturalShapeRockMesh.release, self.naturalShapeRockMesh)
    end
    self.naturalShapeRockMesh = nil
    return baseInvalidate(self)
  end

  local baseDrawVegetation = renderer.drawVegetation
  renderer.drawVegetation = function(self, proj, cmd)
    local vx = variation(cmd, 5) - 0.5
    local vy = variation(cmd, 6) - 0.5
    local dx = vx * 0.090
    local dy = vy * 0.040
    local factor = 0.955 + variation(cmd, 7) * 0.085
  ...
  renderer.drawLowPrism = function(self, proj, cmd, height, topColor)
    ...
    if motif == "boulder" and cmd and cmd.atlasTexture then
      if drawSlopedRock(self, proj, cmd) then
