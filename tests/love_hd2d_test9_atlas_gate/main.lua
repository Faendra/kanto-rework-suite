...
local function fail(message)
  io.stderr:write("FAIL love_hd2d_test9_atlas_gate: " .. tostring(message) .. "\n")
  love.event.quit(1)
end
...
function love.load()
  love.filesystem.setIdentity("krs-hd2d-test9-atlas-gate")
  ...
  local ok, err = pcall(function()
    ...
    -- Bright peripheral pixels ensure TEST9 exercises the silhouette mask.
    for _, id in ipairs(TREE) do
      paintTile(id, {0.82,0.91,0.65}, nil, nil)
      local x, y = tileXY(id)
      love.graphics.setColor(0.08,0.28,0.10,1)
      love.graphics.rectangle("fill", x + 1, y + 2, 6, 5)
      love.graphics.setColor(0.34,0.65,0.21,1)
      love.graphics.rectangle("fill", x + 2, y + 1, 4, 5)
    end
    ...
    local map = {
      id = "SYNTHETIC_TEST9_ATLAS_ROUTE",
    ...
    local state = { map = map, neighbors = {}, entities = { player }, ghosts = {}, player = player }
    local ctx = {
    ...
    local raw = assert(renderer:drawWorld(ctx), "TEST9 renderer returned no canvas")
    saveCanvas(raw, "hd2d-test9-atlas-raw.png")

    assert(forbiddenFlatCalls == 0, "TEST9 invoked flattened Gen1Recomp map rendering")
    assert((renderer.lastAtlasDirectFrames or 0) == 1, "TEST9 direct-atlas path inactive")
    assert((renderer.lastCompatibilityCaptureFrames or 0) == 0, "TEST9 used compatibility capture")
    assert((renderer.lastFlatSourceFallbacks or 0) == 0, "TEST9 leaked flat framebuffer textures")
    assert((renderer.lastAtlasMeshFallbacks or 0) == 0, "TEST9 atlas meshes failed on real LÖVE")
    assert((renderer.lastAtlasGroundCells or 0) >= 50, "TEST9 atlas terrain coverage too low")
    assert((renderer.lastAtlasNaturalObjects or 0) >= 8, "TEST9 atlas natural objects missing")
    assert((renderer.lastAtlasStructures or 0) >= 1, "TEST9 atlas structure missing")
    assert((renderer.lastNaturalSilhouetteBillboards or 0) >= 8, "TEST9 tree billboards missing")
    assert((renderer.lastNaturalSilhouettePixelsCleared or 0) > 0, "TEST9 tree silhouette mask inactive")
    assert((renderer.lastRaisedLawnCells or 0) == 0, "TEST9 still raises lawn semantically")
    assert((renderer.lastTerrainSkirts or 0) == 0, "TEST9 still emits lawn/path skirts")
    assert((renderer.lastFlatOutdoorCells or 0) >= 50, "TEST9 outdoor ground is not continuous")
    assert((renderer.lastScaledTrees or 0) >= 8, "TEST9 tree scale tuning inactive")
    assert((renderer.lastFlattenedBoulders or 0) == 1, "TEST9 boulder flattening inactive")
    assert(renderer.lastActors == 1, "TEST9 player missing")
    ...
    assert(math.abs(focusZ) < 0.001, "TEST9 player ground is unexpectedly elevated")
    ...
    local final = assert(atmosphere:present(raw, ctx, 2, playerY / raw:getHeight()),
                         "TEST9 atmosphere returned no canvas")
    saveCanvas(final, "hd2d-test9-atlas-depth.png")
  end)

  package.preload["hd2d.VanillaMotifs"] = nil
  if not ok then return fail(err) end
  print("PASS love_hd2d_test9_atlas_gate")
  love.event.quit(0)
end
